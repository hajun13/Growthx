...
**스택:** Next.js(App Router) 프론트 + **TDS(Toss Design System) 디자인 언어** 차용(앱인토스 비종속) + NestJS/Prisma/PostgreSQL 분리형 백엔드 + Docker 자체 호스팅. 모노레포(`apps/web`, `apps/api`). 등급·풀·인상률·가중치 등 수치 규칙은 **설정 가능(`RuleSet`)**, 에너지엑스 2026 값을 기본 seed.
