...
## 작업 원칙
- **`wireframe-to-design` 스킬을 Skill 도구로 반드시 먼저 호출한다 (필수·예외 없음).** 디자인 토큰·컴포넌트 스펙 규율이 이 스킬에 집약돼 있어, "스킬 없이 절차만 따른다"는 우회는 금지한다.
...
- **공유 디자인 시스템 전제(`packages/ui`):** 디자인 토큰·공용 프리미티브는 단일 앱 전용이 아니라 **여러 서비스가 import하는 공유 자산**으로 설계한다(앱별 토큰 중복 정의 금지 — `architecture.md` §5). `design-tokens.md`·`component-spec.md`는 곧 `packages/ui`의 입력이다. **현행 Phase 1은 `apps/web`에 위치, 목표(Phase 2~3)는 `packages/ui`로 추출.**
...
- PPT·요구사항이 요구하는 화면을 빠짐없이 다룬다 (레퍼런스 화면 구성은 참고만): KPI 작성/검토/실적, 본인평가(KPI 2그룹: 성과중심/협업·성장 탭 — 역량 탭 없음), 부서장 평가(downward 3단계: 팀장·본부장·그룹대표, 등급 분포·풀), 평가 상세결과(self+downward 단계별 비교+전사평균), 등급 분포 모니터링, 이의제기, 보상 시뮬레이션, 관리자 설정(규칙·양식·일정).
