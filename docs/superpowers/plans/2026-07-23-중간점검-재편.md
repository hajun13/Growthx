...
export function MidtermTrailTimeline({ entries }: { entries: MidtermTrailEntry[] }) {
  if (!entries.length) {
    return (
      <Card>
        <p className="text-sm text-gray-500">아직 진행 이력이 없어요.</p>
      </Card>
    );
  }
  return (
    <Card>
      <h3 className="mb-4 text-base font-semibold">진행 이력</h3>
        ...
        {entries.map((e) => (
          <li key={e.id} className="border-l-2 border-gray-200 pl-4">
            <div className="flex flex-wrap items-baseline gap-2">
              ...
              <span className="text-sm text-gray-700">{ACTION_LABEL[e.action]}</span>
              {e.onBehalfOf && (
                <span className="rounded-sm bg-gray-100 px-1.5 py-0.5 text-xs text-gray-600">
                  인사 대리
                </span>
              )}
            ...
            {e.comment && <p className="mt-1 whitespace-pre-wrap text-sm text-gray-700">{e.comment}</p>}
            {e.kpiChanges.length > 0 && (
              <ul className="mt-2 space-y-1">
                {e.kpiChanges.map((c, i) => (
                  <li key={`${c.kpiId}-${c.field}-${i}`} className="text-sm text-gray-600">
                    <span className="font-medium">{c.kpiTitle}</span> · {FIELD_LABEL[c.field] ?? c.field}{' '}
                    <span className="tabular-nums">{formatValue(c.field, c.before)}</span>
                    {' → '}
                    <span className="font-medium tabular-nums">{formatValue(c.field, c.after)}</span>
                  </li>
                ))}
              </ul>
            )}
    ...
    <div className="space-y-4">
      {kpis.map((kpi) => (
        <Card key={kpi.kpiId}>
          ...
          <p className="mt-1 text-sm text-gray-600">목표: {kpi.targetText ?? kpi.targetValue ?? '-'}</p>
      ...
      <Card>
        <h4 className="mb-2 text-sm font-semibold">전체 총평</h4>
        <textarea
          value={overall}
      ...
      {confirming && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
          <div className="w-full max-w-md rounded-lg bg-white p-6">
            <h3 className="text-base font-semibold">코멘트를 제출할까요?</h3>
            <p className="mt-2 text-sm text-gray-600">
              제출하면 대상자에게 이메일로 안내가 나가고, 대상자가 목표를 수정할 수 있게 돼요.
              조정 필요로 표시한 지표는 {adjustCount}건이에요.
            </p>
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button
                ...
                onClick={submit}
                disabled={saving}
                className="rounded-md bg-[#0257CE] px-4 py-2 text-sm text-white disabled:opacity-50"
              >
                {saving ? '제출 중…' : '제출'}
              </button>
            </div>
  ...
  return (
    <div className="space-y-4">
      {detail.data?.firstComment && (
        <Card>
          <h4 className="text-sm font-semibold">부서장 총평</h4>
          <p className="mt-1 whitespace-pre-wrap text-sm text-gray-700">{detail.data.firstComment}</p>
        </Card>
      )}
        ...
        return (
          <Card key={k.kpiId}>
            <div className="flex flex-wrap items-baseline justify-between gap-2">
              ...
              {needsAdjust && (
                <span className="rounded-sm bg-[#FEF3C7] px-2 py-0.5 text-xs text-[#92400E]">
                  조정 필요
                </span>
              )}
            </div>
            {c?.note && <p className="mt-1 text-sm text-gray-600">부서장: {c.note}</p>}
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <label className="text-sm">
                <span className="mb-1 block text-gray-600">목표</span>
                <input
              ...
              <label className="text-sm">
                <span className="mb-1 block text-gray-600">가중치(%)</span>
                <input
                  type="number"
                  ...
                  className="w-full rounded-md border border-gray-300 px-2 py-1.5 tabular-nums"
                />
              </label>
            </div>
          </Card>
      ...
      <Card>
        <div className="flex items-baseline justify-between">
          <h4 className="text-sm font-semibold">가중치 합계</h4>
          <span className={`text-sm tabular-nums ${weightOk ? 'text-[#0EA05E]' : 'text-[#EF4444]'}`}>
            {weightSum}%
          </span>
        </div>
        {!weightOk && <p className="mt-1 text-xs text-[#EF4444]">합계가 100%가 되어야 제출할 수 있어요.</p>}
      </Card>

      <Card>
        <h4 className="mb-2 text-sm font-semibold">회신 사유</h4>
        <textarea
          value={note}
...
export function FinalReviewPanel({ reviewId, onDone }: { reviewId: string; onDone: () => void }) {
  ...
  return (
    <div className="space-y-4">
      {detail.data?.firstComment && (
        <Card>
          <h4 className="text-sm font-semibold">1차 총평</h4>
          <p className="mt-1 whitespace-pre-wrap text-sm text-gray-700">{detail.data.firstComment}</p>
        </Card>
      )}

      <Card>
        <h4 className="text-sm font-semibold">본인 회신</h4>
        ...
        {lastRevision && lastRevision.kpiChanges.length > 0 ? (
          <ul className="mt-3 space-y-1">
            {lastRevision.kpiChanges.map((c, i) => (
              <li key={`${c.kpiId}-${c.field}-${i}`} className="text-sm text-gray-700">
          ...
          <p className="mt-3 text-sm text-gray-500">KPI 변경 없음</p>
        )}
      </Card>

      <Card>
        <h4 className="mb-2 text-sm font-semibold">검토 의견</h4>
        <textarea
          value={comment}
      ...
      <EvaluationActionPanel
        sticky
        primaryLabel="승인하고 마감"
        onPrimary={() => run('approve')}
        primaryDisabled={saving}
        secondaryLabel="반려"
        onSecondary={() => run('return')}
      />
    </div>
  );
}
...
> `EvaluationActionPanel`의 props 이름(`secondaryLabel`·`onSecondary` 등)은 `apps/web/components/EvaluationActionPanel.tsx`를 열어 확인하고 맞춘다.
...
export function MidtermOpenPanel({ cycleId }: { cycleId: string }) {
  ...
  return (
    <Card>
      <h3 className="text-base font-semibold">중간점검 개시</h3>
      ...
      {preview && (
        <div className="mt-4 space-y-2">
          <p className="text-sm">대상 {preview.targets.length}명</p>
          {preview.warnings.length > 0 && (
            <div className="rounded-md bg-[#FEF3C7] p-3">
              <p className="text-sm font-medium text-[#92400E]">확인이 필요한 항목 {preview.warnings.length}건</p>
              <ul className="mt-1 space-y-0.5">
                {preview.warnings.map((w, i) => (
                  <li key={`${w.userId}-${i}`} className="text-sm text-[#92400E]">
                    {w.name} — {w.reason}
                  </li>
                ))}
              </ul>
      ...
      {result && <p className="mt-3 text-sm text-[#0EA05E]">{result}</p>}
