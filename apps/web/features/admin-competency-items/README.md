...
- **비고:** `useAuth`·`useCurrentCycle`·`useToast` 등 보조 데이터/UI 훅은 기존 유지(주 데이터만 생성 클라이언트로 이관). RBAC(`isHrAdmin`)·라우트·동작·시각 보존. 카테고리 배지 색은 도메인 카테고리(리더십/협업/전문성/혁신)용 `catColors`로 유지(등급 S~D 배지가 아니므로 `lib/grade` 미적용). DESIGN.md(Notion Low Color) 토큰 준수.
