...
// 에러 객체에서 code/message 를 안전하게 읽는다(생성 클라이언트·lib/api 두 ApiError 클래스 호환).
...
export function SelfEvaluationView() {
  ...
  const { data: evidenceData, reload: reloadEvidence } = useSelfEvidence(
    selfEval?.id ?? null,
  );
  ...
  async function save(): Promise<boolean> {
    ...
    try {
      ...
      toast.show({
        variant: 'danger',
        message: errInfo(err).message ?? '저장에 실패했어요.',
      });
      return false;
    }
  }
  ...
  async function handleCreateSelf() {
    ...
    try {
      ...
      toast.show({
        variant: 'danger',
        message:
          errInfo(err).code === 'ALREADY_EXISTS'
            ? '이미 시작한 본인평가가 있어요.'
            : '본인평가를 시작하지 못했어요.',
      });
      reloadEvals();
...
function EvidenceSection({
  ...
  async function handleFiles(fileList: FileList | null) {
    ...
    try {
      ...
      toast.show({
        variant: 'danger',
        message: errInfo(err).message ?? '첨부에 실패했어요.',
      });
    } finally {
      setUploading(false);
  ...
  async function handleDownload(e: EvaluationEvidence) {
    ...
    try {
      ...
      toast.show({
        variant: 'danger',
        message: errInfo(err).message ?? '파일을 열지 못했어요.',
      });
    } finally {
  ...
  async function handleRemove(e: EvaluationEvidence) {
    ...
    try {
      ...
      toast.show({
        variant: 'danger',
        message: errInfo(err).message ?? '삭제에 실패했어요.',
      });
    } finally {
      setRemovingId(null);
    }
