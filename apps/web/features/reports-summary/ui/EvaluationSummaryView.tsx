...
const stickyTh: React.CSSProperties = { ...th, position: 'sticky', top: 0, zIndex: 2 };
...
function StageCells({
  ...
  s: { perf: number | null; comp: number | null };
  tint: string;
}) {
  return (
    <>
...
export function EvaluationSummaryView() {
  ...
  const { rows, loading, error, reload } = useEvaluationSummaryData(
    cycleId,
    !!cycleId,
  );
        ...
        {(['S', 'A', 'B', 'C', 'D'] as Grade[]).map((g) => {
          ...
          return (
            ...
            >
              ...
              <div>
                <span className="tabular-nums" style={{ fontSize: 26, fontWeight: 800, color: c.bg, lineHeight: 1.1 }}>
                  {gradeCounts[g]}
                </span>
            ...
            <thead>
              <tr>
                {['NO', '성명', '그룹', '본부', '팀', '직급', '직책'].map((h) => (
                  <th key={h} rowSpan={2} style={stickyTh}>
                    {h}
                  </th>
                ))}
              ...
              <tr>
                {['실적', '역량', '실적', '역량', '실적', '역량', '실적', '역량'].map((h, i) => (
                  <th key={i} style={{ ...stickyTh, fontWeight: 500 }}>
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {filtered.map((r) => (
                <tr
                  ...
                  onMouseEnter={(e) => {
                    (e.currentTarget as HTMLElement).style.background = K.surfaceLow;
                  }}
                  onMouseLeave={(e) => {
                ...
                >
                  ...
                  <td style={{ ...td, textAlign: 'center' }}>
                    {r.finalGrade ? <GradeBadge grade={r.finalGrade as Grade} /> : '-'}
                  </td>
                </tr>
              ))}
            </tbody>
