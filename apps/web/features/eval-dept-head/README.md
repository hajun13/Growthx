...
- **책임:** 부서장(팀장 1차·본부장 2차·그룹대표 최종, role 식별)이 자기 단계 배정 팀원의 본인평가(self) 실적을 연동 조회하고, 정성 과제 등급·문항별/종합 코멘트·(선택)종합등급 오버라이드를 작성해 제출. 그룹 등급 풀 분포 표시, 본인평가 증빙 인라인 미리보기.
...
- **비고:** `useAuth`·`useCurrentCycle`·`useKpis`·`useGradePools`·`useRuleSet` 등 보조 데이터는 기존 공용 훅 유지(주 데이터=평가만 생성 클라이언트로 이관). RBAC·라우트·데이터 의미·시각/동작 불변. 디자인 토큰은 Kinetic(DESIGN.md).
