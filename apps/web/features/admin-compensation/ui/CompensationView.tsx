...
import type { CompensationSimulation } from '../api';
...
const GRID = '1fr 100px 80px 120px 120px 60px 140px 130px';
...
export function CompensationView() {
  ...
  const { rows } = useTeamCompensationSimulationData(cycleId, allowed && !!cycleId);
    ...
    const body = filtered
      .map((r) => {
        ...
        const gradeCell = grade
          ? `<b style="color:${gradeColor(grade as Grade).fg}">${grade}</b>`
          : '-';
    ...
    <PageContainer>
      ...
      {gradeRaise.length > 0 && (
        <div className="flex items-center gap-3 flex-wrap">
          <span style={{ fontSize: 11.5, color: '#797582', fontWeight: 500 }}>
            등급별 인상률 기준:
          </span>
          <span
            style={{
          ...
          {[...gradeRaise]
            .sort(
              (a, b) =>
                GRADE_ORDER.indexOf(a.grade as Grade) -
                GRADE_ORDER.indexOf(b.grade as Grade),
            )
            .map((g) => {
      ...
      >
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((r, i) => {
            ...
            const diffWon = hasDiff ? r.projectedSalary! - r.currentSalary! : 0;
            const diffMan = Math.round(diffWon / 10000);
            const pct =
              ...
              >
                ...
                <div>
                  ...
                  >
                    ...
                    {r.groupTier && (
                      <span
                        title={`그룹 ${tierLabel[r.groupTier as GroupTier]} · 보너스 ${r.groupTierBonus > 0 ? '+' : ''}${r.groupTierBonus}%p`}
                        style={{
                          fontSize: 10,
                          fontWeight: 700,
                          background: tierBadge[r.groupTier as GroupTier].bg,
                          color: tierBadge[r.groupTier as GroupTier].fg,
                          padding: '1px 6px',
                        }}
                      >
                        {tierLabel[r.groupTier as GroupTier]}
                        {r.groupTierBonus !== 0
                  ...
                  {grade ? (
                    <span
                      style={{
                        fontSize: 12,
                        fontWeight: 700,
                        color: gradeColor(grade as Grade).fg,
                        background: gradeColor(grade as Grade).bg,
                        padding: '3px 10px',
                        borderRadius: 5,
                      }}
                    >
