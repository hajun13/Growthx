...
// 사용자 관리 (hr_admin) — admin-users feature 슬라이스의 View.
// 목록·CRUD(useUsers·userCommands)는 생성 클라이언트(@growthx/contracts) 기반 ../hooks·../api 로 이관.
//   (조직/직급/평가 커맨드는 기존 @/hooks/* 유지 — 본 작업 범위 밖.)
// 목록: GET /users + GET /org-chart(부서 경로·드롭다운). 시각/동작은 기존 페이지 그대로 보존.
...
// 사용자 목록·커맨드는 생성 클라이언트(@growthx/contracts) 기반 슬라이스로 이관.
...
// 조직·직급·평가 커맨드는 @/lib/api ApiError 를, 사용자 커맨드는 contracts ApiError(UserApiError)를 throw.
// 형태(.code/.message)는 같으나 별개 클래스라 instanceof 분기를 각각 맞춘다.
...
export function AdminUsersView() {
  ...
  async function handleAdd(f: FormState) {
    ...
    try {
      ...
      toast.show({
        variant: 'danger',
        message:
          err instanceof UserApiError && err.code === 'ALREADY_EXISTS'
            ? '이미 등록된 이메일이에요.'
  ...
  async function handleEdit(f: FormState) {
    ...
    try {
      ...
      toast.show({
        variant: 'danger',
        message: err instanceof UserApiError ? err.message : '수정에 실패했어요.',
      });
    } finally {
      setSaving(false);
    }
  ...
  async function handleToggleExempt(r: Row) {
    try {
      ...
      toast.show({
        variant: 'danger',
        message: err instanceof UserApiError ? err.message : '처리에 실패했어요.',
      });
    }
  }
  ...
  async function handleResign() {
    ...
    try {
      ...
      toast.show({
        variant: 'danger',
        message: err instanceof UserApiError ? err.message : '퇴사 처리에 실패했어요.',
      });
    } finally {
  ...
  async function handleReactivate() {
    ...
    try {
      ...
      toast.show({
        variant: 'danger',
        message: err instanceof UserApiError ? err.message : '복직 처리에 실패했어요.',
      });
    } finally {
      setLifecycleBusy(false);
    }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (err instanceof UserApiError && err.code === 'CONFLICT') {
        // 이력 차단 — 평탄화 없이 백엔드 message 그대로 인라인 노출.
        setDeleteBlocked(err.message);
      } else {
        toast.show({
          variant: 'danger',
          message: err instanceof UserApiError ? err.message : '삭제에 실패했어요.',
        });
        setDeleteTarget(null);
      }
    } finally {
  ...
  async function handlePurge() {
    ...
    try {
      ...
      toast.show({
        variant: 'danger',
        message: err instanceof UserApiError ? err.message : '완전 삭제에 실패했어요.',
      });
    } finally {
      setLifecycleBusy(false);
    }
  ...
  async function handleMovePerson(userId: string, deptId: string) {
    try {
      ...
      toast.show({
        variant: 'danger',
        message: err instanceof UserApiError ? err.message : '이동에 실패했어요.',
      });
    }
  }
